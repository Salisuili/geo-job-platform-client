...
import LaborerDashboardLayout from '../layouts/LaborerDashboardLayout'; // Still assumes this layout exists
  ...
  useEffect(() => {
    const fetchMyApplications = async () => {
      if (authLoading || !isAuthenticated) {
        // Wait for auth to load, or redirect if not authenticated
        if (!authLoading && !isAuthenticated) {
          console.warn("Unauthorized access attempt to MyApplications. Redirecting.");
          navigate('/login');
        }
        setLoadingApplications(false); // Stop loading if authentication is not ready or failed
        return;
      }

      // Ensure user is a laborer
      if (user?.user_type !== 'laborer') {
        console.warn("Forbidden access attempt to MyApplications by non-laborer. Redirecting.");
        navigate('/laborer/dashboard'); // Or to a generic unauthorized page
        ...
        {applications.length === 0 ? (
          <Card className="text-center py-5 shadow-sm border-0">
            <Card.Body>
              ...
              <Button variant="primary" className="mt-3" onClick={() => navigate('/laborer/find-jobs')}>
                Browse Jobs
              </Button>
            </Card.Body>
          </Card>
                ...
                <Card className="shadow-sm border-0 h-100 d-flex flex-column">
                  {application.job_id ? (
                    <>
                      <Card.Img
                        variant="top"
                        src={application.job_id.image_url ? `${API_BASE_URL}${application.job_id.image_url.startsWith('/uploads/') ? application.job_id.image_url : `/uploads/job_images/${application.job_id.image_url}`}` : defaultJobImage}
                        ...
                        <div className="mt-auto pt-3 border-top">
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() => navigate(`/jobs/${application.job_id._id}`)}
                            >
                              View Job
                            </Button>
                          </div>
